                  ...
                  <div className="relative group cursor-pointer">
                    ...
                    `}>
                      ...
                      <p className={`text-xs font-medium ${formData.imageCover ? "text-orange-700" : "text-slate-500"}`}>
                        {formData.imageCover ? formData.imageCover.name : "Click to upload category image"}
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <tr key={category._id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 flex-shrink-0 relative">
                            <Image 
                              src={category.imageCover ? (category.imageCover.startsWith('http') ? category.imageCover : `http://localhost:8000/img/catagories/${category.imageCover}`) : 'https://placehold.co/100x100?text=Category'}
                              alt={category.name}
                              fill
                          ...
                          <div>
                            <p className="font-semibold text-slate-800 line-clamp-1">{category.name}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 hidden lg:table-cell">
                        <div className="flex items-center gap-2">
                          <span className={`w-1.5 h-1.5 rounded-full ${category.isHidden === "true" ? "bg-amber-400" : "bg-emerald-500"}`} />
                          <span className={`text-xs font-semibold ${category.isHidden === "true" ? "text-amber-600" : "text-emerald-700"}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button 
                            ...
                            title={category.isHidden === "true" ? "Show Category" : "Hide Category"}
                          >
                            {category.isHidden === "true" ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                          </button>
                          <button 
