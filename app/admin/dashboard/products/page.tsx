...
export default function AdminProductsPage() {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: "", description: "", code: "", catagory: "", imageCover: null });
      fetchInitialData();
    } catch (err: any) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage and update your inventory</p>
        </div>
                  ...
                  <div className="relative group cursor-pointer">
                    ...
                    `}>
                      ...
                      <p className={`text-xs font-medium ${formData.imageCover ? "text-orange-700" : "text-slate-500"}`}>
                        {formData.imageCover ? formData.imageCover.name : "Click to upload product image"}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-200">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider hidden md:table-cell">Category</th>
                    ...
                    <tr key={product._id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 flex-shrink-0 relative">
                            <Image 
                              src={product.imageCover ? (product.imageCover.startsWith('http') ? product.imageCover : `http://localhost:8000/img/products/${product.imageCover}`) : 'https://placehold.co/100x100?text=Product'}
                              alt={product.name}
                              fill
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">{product.code || 'No code'}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 hidden lg:table-cell">
                        <div className="flex items-center gap-2">
                          <span className={`w-1.5 h-1.5 rounded-full ${product.isHidden === "true" ? "bg-amber-400" : "bg-emerald-500"}`} />
                          <span className={`text-xs font-semibold ${product.isHidden === "true" ? "text-amber-600" : "text-emerald-700"}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button 
                            ...
                            title={product.isHidden === "true" ? "Show Product" : "Hide Product"}
                          >
                            {product.isHidden === "true" ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                          </button>
                          <button 
